	 ...
	 saveBtn.on("click",function(){
	 	...
	 	if(proContact.attr("validateStatus") == "true" &&
	 		...
	 		if(confirm("是否确认提交数据")){
              ...
              //   var paramsArrays=inputs.serializeArray();
              //   var queryString =$.param(paramsArrays);  //获得读取的值
              //   $.getJSON("../../editProvider",queryString,function(data){
              //       if(data=="1"){
              //           alert("修改成功啦");
              ...
              //       }
              //   });
                $("#providerForm") .ajaxSubmit({
                    type: 'post',
                    url: "../../sys/editProvider",
                    contentType: "application/x-www-form-urlencoded; charset=utf-8",
                    success: function (data) {
                        if (data.count == "1") {
                            alert("修改成功啦");
                            location.href = "../../sys/providerlist.html";
                        } else {
                            alert("修改失败啦");
                            $("#comptyLicPicPath").next().html(data.message);
                            return;
                        }
                    }
                });
