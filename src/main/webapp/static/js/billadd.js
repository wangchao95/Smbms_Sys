...
$(function(){
	...
	$.getJSON("../../sys/proSelect",function(data){
        if(data != null){
	...
	addBtn.on("click",function(){
		if(billCode.attr("validateStatus") != "true"){
			...
			if(confirm("是否确认提交数据")){
				var inputs=$("#billForm").find(":input");  //获得所有的input元素
				var serialArray=inputs.serializeArray();
				var inputString=$.param(serialArray);//获得input元素的name对应的值
				//通过ajax请求
				$.getJSON("../../sys/addBill",inputString,function(data){
					if(data=="1"){
						alert("订单添加成功啦");
						location.href="../../sys/billlist.html";
						return;
					}else{
                        alert("订单添加失败啦");
                       return;
